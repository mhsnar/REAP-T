...
def computeREAP_Lyapanov(Ad = None,Bd = None,Cd = None,Dd = None,Xconstraints = None,Uconstraints = None,x0 = None,Xbar = None,NoS = None,NoI = None,NoO = None,Qx = None,Qu = None,Qv = None,DeltaT = None,Prediction_Horizion = None,Omegastar = None,n = None,C = None): 
...
# x_down=-x_up;
    ...
    Number_of_Constraints = (Prediction_Horizion + 2) * (2 * Xconstraint.shape[1-1] + 2 * Uconstraint.shape[1-1])
    ...
    hat_lambda_omega_x1 = np.zeros((Xconstraint.shape[1-1],1))
    hat_lambda_omega_x2 = np.zeros((Xconstraint.shape[1-1],1))
    hat_lambda_omega_u1 = np.zeros((Uconstraint.shape[1-1],1))
    hat_lambda_omega_u2 = np.zeros((Uconstraint.shape[1-1],1))
...
# theta=[r(1) r(3) r(5)]';
            ...
            # Terminal Conditions
            invpsi = inv(psi)
            err = x_pr(np.arange(Ad.shape[1-1] * Prediction_Horizion - (Ad.shape[1-1] - 1),Ad.shape[1-1] * Prediction_Horizion+1)) - xbar
            LyapanovF = np.transpose((err)) * psi * (err)
            TXConstraints1 = []
            TXConstraints2 = []
            coeff = np.eye(NoS)
            for incc in np.arange(1,NoS+1).reshape(-1):
                TXConstraints1 = np.array([[TXConstraints1],[LyapanovF - ((xbar(incc) - Xconstraint(incc)) ** 2 / (coeff(incc,:) * invpsi * np.transpose(coeff(incc,:)))) + 1 / beta]])
                TXConstraints2 = np.array([[TXConstraints2],[LyapanovF - ((- xbar(incc) + Xconstraint_down(incc)) ** 2 / (coeff(incc,:) * invpsi * np.transpose(coeff(incc,:)))) + 1 / beta]])
            TUConstraints1 = []
            TUConstraints2 = []
            for incc in np.arange(1,NoI+1).reshape(-1):
                TUConstraints1 = np.array([[TUConstraints1],[LyapanovF - ((ubar(incc) - Uconstraint(incc)) ** 2 / (K(incc,:) * invpsi * np.transpose(K(incc,:)))) + 1 / beta]])
                TUConstraints2 = np.array([[TUConstraints2],[LyapanovF - ((- ubar(incc) + Uconstraint_down(incc)) ** 2 / (K(incc,:) * invpsi * np.transpose(K(incc,:)))) + 1 / beta]])
            ...
            if sol.problem != 0:
                raise Exception('The specified prediction horizon length is insufficient for implementing the Lyapunov-based method. Please increase the prediction horizon length.')
        ...
        while toc < AT:
            ...
            dell = np.array([[np.abs(delX) / norm(Bd)],[np.abs(delU)]])
            dellMM = np.amin(dell)
            ...
            while flag_gammaD == 0:
                ...
                if np.any((hat_lambda + ds * Sigma * Checkpoint) < 0) or np.any(Functions.TconUpdate_Lyap(hat_u,ds,Sigma,grad_B_u,Abar,Bbar,x0,psi,Ad,Prediction_Horizion,xbar,ubar,NoS,NoI,Xconstraint,Uconstraint,Uconstraint_down,Xconstraint_down,beta,K) > 0):
                    Sigma = 0.95 * Sigma
                else:
                    flag_gammaD = 1
