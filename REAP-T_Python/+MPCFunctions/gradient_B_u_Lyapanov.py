...
def gradient_B_u_Lyapanov(Bbar = None,errU = None,errX = None,errTX = None,QuBar = None,QxBar = None,NoS = None,hatLambda = None,Prediction_Horizion = None,AllConstraints = None,beta = None,Qn = None,psi = None): 
    ...
    DotLyapunov = (np.transpose(Bbar(np.arange(NoS * Prediction_Horizion - (NoS - 1),NoS * Prediction_Horizion+1),:))) * psi * (errTX)
    den = sum(np.array([[(np.multiply(hat_lambda_omega_x1,(- beta)) / (- beta * TXConstraints1 + 1))],[(np.multiply(hat_lambda_omega_x2,(- beta)) / (- beta * TXConstraints2 + 1))],[(np.multiply(hat_lambda_omega_u1,(- beta)) / (- beta * TUConstraints1 + 1))],[(np.multiply(hat_lambda_omega_u2,(- beta)) / (- beta * TUConstraints2 + 1))]]))
    ...
    grad_B_u = grad_B_u - np.transpose(Bbar) * (np.multiply(np.multiply(hat_lambda_x1,(- beta)),Ax1) / (- beta * XConstraints1 + 1)) - np.transpose(Bbar) * (np.multiply(np.multiply(hat_lambda_x2,(- beta)),Ax2) / (- beta * XConstraints2 + 1)) - np.multiply(np.multiply(hat_lambda_u1,(- beta)),Cu1) / (- beta * UConstraints1 + 1) - np.multiply(np.multiply(hat_lambda_u2,(- beta)),Cu2) / (- beta * UConstraints2 + 1) - np.multiply(DotLyapunov,den)
