function [x, u_app,Sigmas] = computeMPCLyapanov(Ad, Bd, Cd,Dd,Xconstraints,Uconstraints,x0,Xbar, NoS, NoI, NoO, Qx, Qu, Qv, DeltaT, Prediction_Horizion, Omegastar, n,C)
...
for inc=1:n
    ...
    while toc<AT
        ...
        [minValue, minIndex] = max([delX, delU]);
        ...
        flag_gammaD=0;
        while flag_gammaD==0
            if any([gammaD*dell(minIndex);gammaD*dell(minIndex)]>dell)
                gammaD=gammaD-0.1;
            else
                flag_gammaD=1;
            end
        ...
        while flag_gammaD==0
            if any((hat_lambda+ds*Sigma*Checkpoint)<0) || any(Functions.TconUpdate_Lyap(hat_u,ds,Sigma,grad_B_u,Abar,Bbar,x0,psi,Ad,Prediction_Horizion,xbar,ubar,NoS,NoI,Xconstraint,Uconstraint,Uconstraint_down,Xconstraint_down,beta,K)>0)
                ...
                Tcon=Functions.TconUpdate_Lyap(hat_u,ds,Sigma,grad_B_u,Abar,Bbar,x0,psi,Ad,Prediction_Horizion,xbar,ubar,NoS,NoI,Xconstraint,Uconstraint,Uconstraint_down,Xconstraint_down,beta,K);
                cacac=hat_lambda+ds*Sigma*Checkpoint;
                hattttu=hat_u;
                s=hattttu;
            else
                flag_gammaD=1;
        ...
        y=norm(grad_B_lambda+ Functions.Phi(hatLambda,grad_B_lambda,hat_lambda));
